"""
Tiny interactive coding agent with a terminal UI and structured tools.

Run without --task to start the interactive TUI. Natural-language input is
sent to the coding agent; prefix a line with ! to execute a shell command
immediately. Sessions can be saved and restored with /save and /load.

Examples:
  python tiny_coding_tui.py --workdir /tmp/project
  python tiny_coding_tui.py --workdir /tmp/project --task "Create hello.py" --tui
  python tiny_coding_tui.py --workdir /tmp/project --task "Run tests"  # one-shot
"""
...
- Make reasonable assumptions and proceed; do not stop to ask the user questions. Keep any \
intermediate narration concise and focused on concrete progress.
...
class AgentCancelled(KeyboardInterrupt):
    """Raised when Ctrl+C stops the current agent or terminal operation."""


class CancellationToken:
    """Tracks cancellation and the currently running subprocess, if any."""
...
class Workspace:
    def __init__(self, root: str, max_output_chars: int = DEFAULT_MAX_OUTPUT_CHARS,
                 cancellation: Optional[CancellationToken] = None) -> None:
    ...
    def terminal(self, command: str, timeout: int = 60) -> Dict[str, Any]:
        """Run a shell command and make Ctrl+C terminate its whole process group."""
        started = time.time()
        ...
        try:
            ...
            if self.cancellation:
                self.cancellation.register_process(process)
            ...
            while True:
                ...
                if elapsed >= timeout:
                    if self.cancellation:
                        self.cancellation._terminate(process)
                    else:
                        CancellationToken._terminate(process)
                    try:
            ...
            if process is not None and self.cancellation:
                self.cancellation.clear_process(process)
...
class TokenCounter:
    """Count context tokens with tiktoken when available, else use a stable estimate."""
    ...
    def count_text(self, text: str) -> int:
        ...
        # A practical fallback for mixed code/prose. It deliberately rounds up.
        return max(1, (len(text) + 3) // 4)

    def count_payload(self, messages: List[Dict[str, Any]], tools: List[Dict[str, Any]]) -> int:
...
def _safe_display_args(name: str, args: Dict[str, Any]) -> Dict[str, Any]:
    """Keep TUI tool call previews useful without dumping whole file contents."""
    display = dict(args)
    for key in ("content", "old_text", "new_text"):
        value = display.get(key)
        if isinstance(value, str) and len(value) > 300:
            display[key] = value[:180] + f"… <{len(value)} chars total>"
    if name == "terminal" and isinstance(display.get("command"), str):
        command = display["command"]
        if len(command) > 1200:
            display["command"] = command[:1200] + "…"
    return display


class TUIRenderer:
    """Rich output when available, with a dependency-free plain fallback."""
    ...
    def banner(self, session: "InteractiveCodingSession") -> None:
        title = "Tiny Coding Agent TUI"
        details = (
            f"Workspace: {session.workspace.root}\n"
            f"Model: {session.args.model}  |  Tools: {', '.join(session.enabled_tool_names) or 'none'}\n"
            "Enter a coding task, !<command> for direct shell, or /help for commands."
        )
        if self.rich:
            from rich.panel import Panel
            self.console.print(Panel(details, title=title, border_style="cyan"))
        else:
            self._print(f"=== {title} ===\n{details}\n")
    ...
    def assistant(self, text: str) -> None:
        ...
        if self.rich:
            ...
            from rich.panel import Panel
            self.console.print(Panel(Markdown(text), title="Agent", border_style="green"))
        else:
            self._print(f"\nAgent> {text}")

    def tool_start(self, name: str, args: Dict[str, Any]) -> None:
        shown = json.dumps(_safe_display_args(name, args), ensure_ascii=False, indent=2)
        if self.rich:
            from rich.panel import Panel
            self.console.print(Panel(shown, title=f"Tool · {name}", border_style="yellow"))
        else:
            self._print(f"\n[tool:{name}] {shown}")
    ...
    def tool_result(self, name: str, result: Dict[str, Any]) -> None:
        ...
        if name == "terminal":
            ...
            suffix = f"exit={result.get('exit_code')} · {result.get('elapsed_sec', '?')}s"
            if result.get("error"):
                suffix += f" · {result['error']}"
            body = output.rstrip() or "(no output)"
        elif name == "read_file":
            body = (
                f"{result.get('path', '')}: lines {result.get('start_line', '?')}–"
                f"{result.get('end_line', '?')} of {result.get('total_lines', '?')}"
            ) if ok else str(result.get("error") or result)
            suffix = "read"
        elif name == "search_files":
            body = f"{result.get('match_count', 0)} matches in {result.get('files_scanned', 0)} files"
            suffix = "search"
        elif name == "list_files":
            body = f"{len(result.get('entries') or [])} entries"
            suffix = "listing"
        elif name == "web_search":
            rows = []
            for item in (result.get("results") or [])[:8]:
                rows.append(f"• {item.get('title', '')}\n  {item.get('url', '')}")
            body = "\n".join(rows) or str(result.get("error") or "No results")
            suffix = str(result.get("source") or "search")
        else:
            selected = {
                key: value for key, value in result.items()
                if key in {"ok", "path", "bytes", "append", "deleted", "occurrences_found",
                           "occurrences_replaced", "error", "hint", "workdir", "final_url", "title"}
            }
            body = json.dumps(selected or result, ensure_ascii=False, indent=2, default=str)
            suffix = "ok" if ok else "failed"

        if self.rich:
            from rich.panel import Panel
            border = "green" if ok else "red"
            self.console.print(Panel(body, title=f"Result · {name} · {suffix}", border_style=border))
        else:
            self._print(f"[result:{name}:{'ok' if ok else 'failed'}] {body}")
...
class InteractiveCodingSession:
    SESSION_FORMAT = "tiny-coding-tui-session"
    SESSION_VERSION = 1

    def __init__(self, args: argparse.Namespace, renderer: TUIRenderer) -> None:
        self.args = args
        ...
        self.enabled_tool_names = resolve_enabled_tool_names(args.tools)
        self.enabled_tools = filter_tools(self.enabled_tool_names)
        self.enabled_tool_set = set(self.enabled_tool_names)
        self.web_config = {
            "timeout": args.web_timeout,
            "max_results": args.web_max_results,
            "max_chars": args.web_max_chars,
            "allow_local": args.allow_local_fetch,
            "user_agent": WEB_USER_AGENT,
            "searxng_url": args.searxng_url or "",
        }
        self.token_counter = TokenCounter(args.model)
        self.last_api_prompt_tokens: Optional[int] = None
        self.last_api_total_tokens: Optional[int] = None
        self.last_saved_path: Optional[Path] = None
        self.logger = TurnLogger(getattr(args, "log_file", None), {
            "mode": "tui",
            "model": args.model,
    ...
    def _initial_system_message(self) -> Dict[str, Any]:
        prompt = build_system_prompt(self.enabled_tool_names, str(self.workspace.root))
        prompt += (
            "\n\nInteractive session rules:\n"
            "- The user may submit follow-up tasks. Preserve useful context from earlier turns.\n"
            "- A user message beginning with [Direct shell command] records a command the user ran.\n"
            "- Ctrl+C can interrupt your current work; do not claim interrupted work was completed."
        )
        project = self.workspace.load_project_instructions()
        if project:
            prompt += (
                f"\n\nProject instructions from {project['file']}:\n"
                f"----- BEGIN {project['file']} -----\n"
                f"{project['content']}\n"
                f"----- END {project['file']} -----"
            )
    ...
    def new_session(self, announce: bool = True) -> None:
        self.messages = [self._initial_system_message()]
        self.last_api_prompt_tokens = None
        self.last_api_total_tokens = None
        self.last_saved_path = None
        self.logger.log("new_session", system_prompt=self.messages[0]["content"])
        if announce:
            self.renderer.info("Started a new session.")

    def context_tokens(self) -> int:
        return self.token_counter.count_payload(self.messages, self.enabled_tools)

    def context_status(self) -> str:
        used = self.context_tokens()
        limit = max(1, int(self.args.context_window))
        percent = used * 100.0 / limit
        marker = "" if self.token_counter.method == "tiktoken" else "~"
        return (
            f"ctx {marker}{_compact_number(used)}/{_compact_number(limit)} "
            f"({percent:.1f}%) · {len(self.messages)} msgs · {self.args.model}"
        )

    def context_details(self) -> str:
        estimated = self.context_tokens()
        lines = [
            f"Current context: {'approximately ' if self.token_counter.method != 'tiktoken' else ''}"
            f"{estimated:,} / {self.args.context_window:,} tokens "
            f"({estimated * 100.0 / max(1, self.args.context_window):.2f}%)",
            f"Counting method: {self.token_counter.method}",
            f"Messages retained: {len(self.messages)}",
        ]
        if self.last_api_prompt_tokens is not None:
            lines.append(f"Last API-reported prompt: {self.last_api_prompt_tokens:,} tokens")
        if self.last_api_total_tokens is not None:
            lines.append(f"Last API-reported total: {self.last_api_total_tokens:,} tokens")
        return "\n".join(lines)

    def _update_api_usage(self, data: Dict[str, Any]) -> None:
        usage = data.get("usage") or {}
        prompt_tokens = usage.get("prompt_tokens")
        total_tokens = usage.get("total_tokens")
        if isinstance(prompt_tokens, int):
            self.last_api_prompt_tokens = prompt_tokens
        if isinstance(total_tokens, int):
            self.last_api_total_tokens = total_tokens

    def _append_cancelled_tool_results(self, calls: List[Dict[str, Any]], start: int) -> None:
        for idx in range(start, len(calls)):
            call_id, name, _ = normalize_tool_call(calls[idx], idx)
            result = {"ok": False, "cancelled": True, "error": "Stopped by user with Ctrl+C"}
            ...
            self.logger.log("tool_result", call_id=call_id, name=name, result=result)

    def run_turn(self, user_text: str) -> Optional[str]:
        user_text = user_text.strip()
        if not user_text:
            return None
        self.cancellation.reset()
        self.messages.append({"role": "user", "content": user_text})
        self.logger.log("user", content=user_text)
        ...
        try:
            for iteration in range(1, self.args.max_iterations + 1):
                ...
                self._update_api_usage(data)
                message = ((data.get("choices") or [{}])[0].get("message") or {})
                content = message.get("content") or ""
                native_calls = message.get("tool_calls") or []
                tool_calls = native_calls or parse_text_tool_calls(content) or []
                self.logger.log(
                    "model_response",
                    iteration=iteration,
                    content=content,
                ...
                if content:
                    last_text = content
                    # Suppress fallback JSON tool envelopes, but show normal plans and final text.
                    if native_calls or not tool_calls:
                        self.renderer.assistant(content)

                if not tool_calls:
                    self.messages.append({"role": "assistant", "content": content})
                    self.logger.log("final", iteration=iteration, content=content)
                    self.renderer.info(self.context_status())
                    return content
                ...
                for idx, call in enumerate(tool_calls):
                    call_id, name, tool_args = normalize_tool_call(call, idx)
                    self.renderer.tool_start(name, tool_args)
                    self.logger.log(
                        "tool_call", iteration=iteration, call_id=call_id, name=name, args=tool_args
                    )
                    try:
                        self.cancellation.check()
                        if name not in self.enabled_tool_set:
                            result = {
                                "ok": False,
                        ...
                        result = {
                            "ok": False,
                            "cancelled": True,
                            "error": "Stopped by user with Ctrl+C",
                        }
                        self.messages.append({
                            "role": "tool",
                            "tool_call_id": call_id,
                        ...
                        self.logger.log("tool_result", call_id=call_id, name=name, result=result)
                        self._append_cancelled_tool_results(tool_calls, idx + 1)
                        raise AgentCancelled()

                    self.renderer.tool_result(name, result)
                    self.logger.log(
                        "tool_result", iteration=iteration, call_id=call_id, name=name, result=result
                    )
                    if name == "terminal" and COMPLETION_MARKER in str(result.get("output", "")):
                        marker_seen = True
                    self.messages.append({
                ...
                if marker_seen:
                    self.messages.append({
                        "role": "user",
                        "content": "The completion marker was observed. Provide the final concise answer.",
                    })

            stopped = (
                f"Stopped after max_iterations={self.args.max_iterations} without a final answer."
            )
            self.messages.append({"role": "assistant", "content": stopped})
            self.logger.log("stopped", reason="max_iterations", last_text=last_text)
            self.renderer.warning(stopped)
            self.renderer.info(self.context_status())
            return last_text or stopped
        except (AgentCancelled, KeyboardInterrupt):
            self.cancellation.cancel()
            interruption = "Execution was interrupted by the user with Ctrl+C."
            self.messages.append({"role": "assistant", "content": interruption})
            self.logger.log("stopped", reason="ctrl_c")
            self.renderer.warning(interruption)
            self.renderer.info(self.context_status())
            return None
        except Exception as exc:
            error_text = f"Agent error: {exc}"
            self.messages.append({"role": "assistant", "content": error_text})
            self.logger.log("error", where="interactive_turn", error=str(exc))
            self.renderer.error(error_text)
            return None

    def run_direct_shell(self, command: str) -> None:
        command = command.strip()
        if not command:
            self.renderer.warning("Use ! followed by a command, for example: !pytest -q")
            return
        self.cancellation.reset()
        self.renderer.tool_start("terminal", {"command": command, "timeout": self.args.command_timeout})
        try:
            result = self.workspace.terminal(command, self.args.command_timeout)
            self.renderer.tool_result("terminal", result)
            record = {
                "role": "user",
                "content": f"[Direct shell command]\n$ {command}",
            }
            observation = {
                "role": "assistant",
                "content": (
            ...
            self.messages.extend([record, observation])
            self.logger.log("direct_shell", command=command, result=result)
            self.renderer.info(self.context_status())
        except (AgentCancelled, KeyboardInterrupt):
            self.cancellation.cancel()
            self.renderer.warning("Shell command stopped with Ctrl+C.")
            self.logger.log("stopped", reason="ctrl_c", where="direct_shell", command=command)

    def _sessions_dir(self) -> Path:
        directory = self.workspace.root / ".coding-agent" / "sessions"
        directory.mkdir(parents=True, exist_ok=True)
        return directory

    def _resolve_save_path(self, raw: str = "") -> Path:
        raw = raw.strip()
        if not raw:
            stamp = datetime.now().strftime("%Y%m%d-%H%M%S")
            return self._sessions_dir() / f"session-{stamp}.json"
        path = Path(raw).expanduser()
        if path.is_absolute():
            resolved = path
        elif "/" in raw or "\\" in raw or raw.startswith("."):
            resolved = (self.workspace.root / path).resolve()
        else:
            resolved = self._sessions_dir() / path
        if not resolved.suffix:
            resolved = resolved.with_suffix(".json")
        return resolved

    def _resolve_load_path(self, raw: str = "") -> Path:
        raw = raw.strip()
        if raw:
            candidates = [self._resolve_save_path(raw)]
            direct = Path(raw).expanduser()
            if direct.is_absolute():
                candidates.insert(0, direct)
            else:
                candidates.insert(0, (self.workspace.root / direct).resolve())
            for candidate in candidates:
                if candidate.is_file():
                    return candidate
            return candidates[-1]
        if self.last_saved_path and self.last_saved_path.is_file():
            return self.last_saved_path
        files = sorted(self._sessions_dir().glob("*.json"), key=lambda p: p.stat().st_mtime, reverse=True)
        if not files:
            raise FileNotFoundError("No saved sessions found. Use /load <file>.")
        return files[0]

    def save(self, raw_path: str = "") -> Path:
        path = self._resolve_save_path(raw_path)
        ...
        self.logger.log("session_saved", path=str(path), messages=len(self.messages))
        return path

    def load(self, raw_path: str = "") -> Path:
        path = self._resolve_load_path(raw_path)
        data = json.loads(path.read_text(encoding="utf-8"))
        if data.get("format") != self.SESSION_FORMAT:
            raise ValueError(f"Unsupported session format in {path}")
        if int(data.get("version", 0)) > self.SESSION_VERSION:
            raise ValueError(f"Session version {data.get('version')} is newer than supported")
        messages = data.get("messages")
        if not isinstance(messages, list) or not messages:
            raise ValueError("Saved session has no messages")
        ...
        self.last_saved_path = path
        saved_workspace = str(data.get("workspace") or "")
        if saved_workspace and Path(saved_workspace).expanduser().resolve() != self.workspace.root:
            self.renderer.warning(
                f"Session was saved for {saved_workspace}; tools will use current workspace {self.workspace.root}."
            )
        saved_model = str(data.get("model") or "")
        if saved_model and saved_model != self.args.model:
            self.renderer.warning(
                f"Session used model {saved_model}; continuing with current model {self.args.model}."
            )
        self.logger.log("session_loaded", path=str(path), messages=len(self.messages))
        return path
...
def run_tui(args: argparse.Namespace) -> int:
    try:
        ...
        print(
            "Interactive mode requires prompt_toolkit. Install dependencies with:\n"
            "  pip install prompt_toolkit rich requests ddgs",
            file=sys.stderr,
        )
        return 1

    renderer = TUIRenderer(no_color=args.no_color)
    session = InteractiveCodingSession(args, renderer)
    if args.session:
        try:
            path = session.load(args.session)
            renderer.info(f"Loaded session: {path}")
        except Exception as exc:
            renderer.error(f"Could not load session: {exc}")
            return 1

    renderer.banner(session)
    history_path = Path(args.history_file).expanduser() if args.history_file else (
        session.workspace.root / ".coding-agent" / "prompt-history"
    )
    history_path.parent.mkdir(parents=True, exist_ok=True)
    prompt = PromptSession(history=FileHistory(str(history_path)))

    def bottom_toolbar() -> Any:
        status = html_lib.escape(session.context_status())
        return HTML(f" <b>{status}</b>  ·  Ctrl+C stop  ·  /exit quit ")

    def execute_input(text: str) -> bool:
        stripped = text.strip()
        if not stripped:
            return True
        if stripped.startswith("!"):
            session.run_direct_shell(stripped[1:])
            return True
        if not stripped.startswith("/"):
            session.run_turn(stripped)
            return True

        command, argument = _parse_slash_command(stripped)
        if command in {"/exit", "/quit", "/q"}:
            return False
        if command == "/help":
            renderer.info(
                "Commands:\n"
                "  /save [name-or-path]  Save the complete conversation\n"
                "  /load [name-or-path]  Load a session; without a path loads the latest\n"
                "  /new                  Start a fresh conversation\n"
                "  /context              Show token/context utilization\n"
                "  /exit                 Quit\n"
                "  !<command>             Run a shell command directly in the workspace\n"
            )
        elif command == "/save":
            try:
                # shlex lets paths be quoted while still accepting a simple unquoted name.
                parsed = shlex.split(argument)
                raw = parsed[0] if parsed else ""
                path = session.save(raw)
                renderer.info(f"Session saved: {path}")
            except Exception as exc:
                renderer.error(f"Could not save session: {exc}")
        elif command == "/load":
            try:
                parsed = shlex.split(argument)
                raw = parsed[0] if parsed else ""
                path = session.load(raw)
                renderer.info(f"Session loaded: {path}")
                renderer.info(session.context_details())
            except Exception as exc:
                renderer.error(f"Could not load session: {exc}")
        elif command == "/new":
            session.new_session()
            ...
            renderer.warning(f"Unknown command: {command}. Use /help.")
        return True

    if args.task:
        execute_input(args.task)
    ...
    while True:
        try:
            ...
            renderer.warning("Nothing is running. Use /exit to quit.")
            continue
        except EOFError:
            renderer.info("Exiting.")
            break
        if not execute_input(text):
            renderer.info("Session closed. Use /save before /exit when you need to continue later.")
            break
    return 0

def main() -> int:
    parser = argparse.ArgumentParser(
        description="Interactive coding agent TUI with terminal, file, and web tools."
    )
    parser.add_argument(
        "--task",
        help="Optional initial task. Without --tui, providing --task keeps one-shot CLI behavior.",
    )
    parser.add_argument(
        "--tui",
        action="store_true",
        help="Stay in the interactive TUI after an optional --task.",
    )
    parser.add_argument(
        "--workdir",
        default=os.getcwd(),
    ...
    parser.add_argument(
        ...
        help=(
            "Tool set: all, terminal, files, web, none, or comma-separated names. "
            "Examples: --tools terminal ; --tools read_file,write_file,edit_file"
        ),
    )
    ...
    parser.add_argument("--max-iterations", type=int, default=20,
                        help="Maximum model/tool iterations for each user task.")
    ...
    parser.add_argument(
        "--context-window",
        type=int,
        default=int(os.getenv("CODING_CLI_CONTEXT_WINDOW", "128000")),
        help="Model context window used for the utilization indicator (default: 128000).",
    )
    parser.add_argument(
        "--session",
        help="Load a saved TUI session at startup.",
    )
    parser.add_argument(
        "--history-file",
        default=os.getenv("CODING_CLI_HISTORY_FILE", ""),
        help="Prompt history file. Defaults to .coding-agent/prompt-history in the workspace.",
    )
    parser.add_argument("--no-color", action="store_true", help="Disable Rich colors.")
    ...
    parser.add_argument("--allow-local-fetch", action="store_true",
                        help="Allow web_fetch to reach private/loopback hosts (off by default).")
    parser.add_argument("--searxng-url", default=os.getenv("CODING_CLI_SEARXNG_URL", ""),
                        help="SearXNG fallback base URL.")
    parser.add_argument(
        "--log-file",
        default=os.getenv("CODING_CLI_LOG_FILE"),
        help="Append a JSONL transcript of model/tool activity to this file.",
    )
    ...
    # No task means interactive mode. --tui keeps the interface open after an initial task.
